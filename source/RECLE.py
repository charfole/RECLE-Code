...
class RECLE(object):
    ...
    def find_similar_sample(self, X, V_miss, distance_metric='cosine', top_k=3):
        ...
        for each v in V_miss, find the closest top_k x in X
        args: 
        ...
        for v in V_miss:
            ...
            target += [x for x in np.argsort(distance)[:top_k]] # 返回排序后的index
    ...
    def buildRLL(self, lr_rate, reg_scale, dropout_rate, activation):
        ...
        # 设置一个初始的tensor，该变量代表is_training的flag，一开始默认用False初始化，name则用于可视化Tensorboard
        ...
        # 这里为四对原始向量的距离进行初始化，特征学习模型的目标是让正样本之间距离变小，而正负样本之间距离变大
        ...
        # 这里为四对原始数据的距离计算值，特征学习模型的目标是让正样本之间距离变小，而正负样本之间距离变大
        ...
        # 定义一个命名空间来代表某一层
        # query代表轮询样本，PosDoc代表另一个正样本，outputNegDoc为三个负样本
        ...
        with tf.name_scope('loss'):
            ...
            # 这里是模型的核心构造部分，numerator代表两个正向量的余弦距离再乘上gamma来作为指数，其后再乘一个权重作为概率公式中的一项
    ...
    def train_and_evaluate(self, train, validation, batchSize, max_iter, 
        ...
        if(not os.path.exists(currentModelPath)):
            ...
            # 修改路径为os.path.join()模式
            os.makedirs(os.path.join(currentModelPath,'current'))
            os.makedirs(os.path.join(currentModelPath,'best_acc_model'))
        ...
        with tf.Session() as sess:
            ...
            val_writer = tf.summary.FileWriter(os.path.join(summaries_dir,'validation'), sess.graph)
            ...
            for epoch in range(0, max_iter):
                ...
                # 修改路径为os.path.join()模式
                saver.save(sess, os.path.join(os.path.join(currentModelPath,'current'), 'RECLE.ckpt'))
                ...
                if(epoch%2==0):
                    ...
                    if(acc>best_acc):
                        ...
                        # 修改路径为os.path.join()模式
                        saver.save(sess, os.path.join(os.path.join(currentModelPath,'best_acc_model'), 'RECLE.ckpt'))
